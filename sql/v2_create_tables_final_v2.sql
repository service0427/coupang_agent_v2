-- V2 테이블 생성 스크립트 (최종 버전 v2)
-- Chrome 전용 쿠팡 자동화 프로젝트
-- 
-- 변경사항:
-- 1. userdata=true, session=false, cache=true 가 기본 정책
-- 2. v1_agent_config의 도메인별 설정을 v2_keywords 테이블에 직접 통합
-- 3. search_mode는 keywords 테이블에서 제거 (자동 전환 시스템)
...
-- v2_test_keywords 테이블 (v1_agent_config 스타일 통합)
CREATE TABLE v2_test_keywords (
    ...
    -- 도메인별 allow 설정 (v1_agent_config 통합)
...
CREATE TABLE v2_execution_logs (
    ...
    -- 검색 모드 관련 (자동 전환 시스템으로 기록)
    search_mode VARCHAR(20),  -- 'goto' or 'search' (기본값: goto, 자동 전환)
    search_mode_reason VARCHAR(50),  -- 'initial', 'auto_switch_blocked', 'auto_switch_rotation'
...
-- 코멘트 추가
COMMENT ON TABLE v2_test_keywords IS 'V2 키워드 테이블 - v1_agent_config 스타일 도메인 설정 통합 (기본 정책: userdata=true, session=false, cache=true)';
COMMENT ON COLUMN v2_test_keywords.coupang_main_allow IS 'www.coupang.com 허용 리소스 타입 (JSON 배열) - NULL이면 기본값 사용';
COMMENT ON COLUMN v2_test_keywords.front_cdn_allow IS 'front.coupangcdn.com 허용 리소스 타입 - ex: ["script", "stylesheet"]';
COMMENT ON COLUMN v2_test_keywords.coupang_main_block_patterns IS 'URL 패턴 기반 차단 규칙 - ex: ["/api/tracking", "/log"]';

COMMENT ON TABLE v2_execution_logs IS 'V2 실행 로그 - 적용된 최적화 설정 및 검색 모드 포함';
COMMENT ON COLUMN v2_execution_logs.optimize_config_applied IS '실행 시 적용된 실제 도메인 규칙 (JSON)';
COMMENT ON COLUMN v2_execution_logs.search_mode IS '실행 시 사용된 검색 모드 - goto(URL직접) or search(검색창), 자동 전환됨';
COMMENT ON COLUMN v2_execution_logs.search_mode_reason IS '검색 모드 선택/전환 이유';

-- 샘플 데이터 입력 (v1_agent_config 스타일의 도메인별 설정)
INSERT INTO v2_test_keywords (keyword, suffix, product_code, agent, coupang_main_allow, notes) 
VALUES 
    -- 기본 설정 (NULL = 하드코딩 기본값 사용)
    ('노트북', NULL, '76174145', 'default', NULL, '기본 최적화 설정 - 하드코딩 값 사용'),
    
    -- 최소 리소스 테스트 (document만)
    ('노트북', '게이밍', '87654321', 'win11', '["document"]', 'document만 허용 - 최소 리소스 테스트'),
    
    -- API 포함 테스트 (기본 설정과 동일)
    ('노트북', '업무용', '12345678', 'u24', '["document", "xhr", "fetch"]', 'document + API 허용 - 기본 설정'),
    
    -- 전체 허용 테스트
    ('맥북', 'M2', '11111111', 'vm', '["*"]', '모든 리소스 허용 - 패턴 탐지 회피 테스트'),
    
    -- CDN 설정 포함 테스트
    ('아이패드', '프로', '22222222', 'local', '["document", "xhr", "fetch"]', 'main은 기본, CDN은 script/css 허용');
...
SELECT 
    ...
    k.agent,
    CASE 
        WHEN k.coupang_main_allow IS NOT NULL THEN 'custom'
        ELSE 'default'
    END as config_type,
    COALESCE(k.coupang_main_allow, 'default') as main_allow_config,
...
GROUP BY config_type, k.coupang_main_allow
...
-- 검색 모드 분석 뷰 (자동 전환 효과)
CREATE OR REPLACE VIEW v2_search_mode_analysis AS
SELECT 
    e.agent,
    e.search_mode,
    e.search_mode_reason,
    COUNT(*) as execution_count,
    COUNT(CASE WHEN e.success = true THEN 1 END) as success_count,
    COUNT(CASE WHEN e.is_blocked = true THEN 1 END) as blocked_count,
...
GROUP BY e.agent, e.search_mode, e.search_mode_reason
