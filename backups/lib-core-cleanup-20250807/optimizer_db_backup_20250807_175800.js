...
// 무조건 허용되는 도메인들 (최적화 설정과 무관하게 항상 허용)
...
function buildDomainRulesFromConfig(agentConfig) {
  ...
  // 기타 도메인들 설정 (NULL이면 기본값 사용, 무조건 허용 도메인들 제외)
  const domainMappings = [
    { config: 'mercury_allow', domain: 'mercury.coupang.com', defaultAllow: [] },
    { config: 'image_cdn_allow', domain: 'image*.coupangcdn.com', defaultAllow: [] },
    { config: 'static_cdn_allow', domain: 'static.coupangcdn.com', defaultAllow: [] },
...
async function applyDynamicOptimization(page, agent = null, keywordData = null) {
  ...
  console.log(`🔍 ${idPrefix}[OptimizerDB] V2 키워드 기반 동적 최적화 시작`);
  ...
  await page.route('**/*', async (route) => {
    ...
    // 1단계: 무조건 허용 도메인 체크
    const isAlwaysAllowed = ALWAYS_ALLOWED_DOMAINS.includes(domain);
    if (isAlwaysAllowed) {
      allowedCount++;
      await route.continue();
      return;
    ...
    if (isAllowed) {
      ...
      // 허용된 요청 로그 출력
      console.log(`✅ ${idPrefix}허용: ${domain} | ${resourceType} | ${pathname}`);
      ...
      // 차단 로그 출력
      console.log(`🚫 ${idPrefix}차단: ${domain} | ${resourceType} | ${pathname}`);
    }
  });
  ...
  return () => {
    isActive = false;
    // 간소화된 통계만 출력
    console.log(`\n📊 ${idPrefix}DB 기반 최적화 완료: 허용 ${allowedCount}개, 차단 ${blockedCount}개 (${((blockedCount / (allowedCount + blockedCount)) * 100).toFixed(1)}%)`);
