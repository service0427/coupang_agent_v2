...
class NetworkMonitor {
      ...
      this.cdpSession.on('Network.responseReceived', (params) => {
        ...
        if (!this.requestMap.has(requestId)) {
          ...
          if (fromCache) {
            ...
            const sizeKB = (response.encodedDataLength || 0) / 1024;
            console.log(`   💾 ${cacheTypeDisplay} Cache 히트 (${sizeKB.toFixed(1)}KB 절약): ${response.url.substring(0, 60)}...`);
          } else {
            // 캐시 미스도 크기와 함께 출력 (큰 파일만)
            const sizeKB = (response.encodedDataLength || 0) / 1024;
            if (sizeKB > 50) { // 50KB 이상인 파일만 표시
              console.log(`   🌐 Network 로드 (${sizeKB.toFixed(1)}KB): ${response.url.substring(0, 60)}...`);
            }
          }
        }
      });
  ...
  getData() {
    ...
    return {
      ...
      totalSize: totalSize,
      networkOnlySize: networkOnlySize,  // 실제 네트워크 사용량
      cachedSize: cachedSize,           // 캐시로 절약된 용량
      ...
      cacheStats: {
        ...
        cacheHitRate: this.cacheStats.total > 0 
          ...
          : 0,
        cacheSavingsMB: (cachedSize / (1024 * 1024)).toFixed(2),
        networkOnlyMB: (networkOnlySize / (1024 * 1024)).toFixed(2),
  ...
  /**
   * 성능 메트릭용 통계 데이터 반환 (api-mode.js에서 사용)
   */
  getStats() {
    const data = this.getData();
    
    // api-mode.js가 기대하는 형태로 변환
    return {
      totalRequests: data.totalRequests,
      blockedRequests: 0, // 현재 차단 요청 추적 안 함
      totalBytes: data.totalSize,
      cacheHits: data.cacheStats.fromCache,
      networkOnlyBytes: data.networkOnlySize,
      cachedBytes: data.cachedSize,
      // 추가 메트릭
      cacheHitRate: parseFloat(data.cacheStats.cacheHitRate) / 100, // 백분율을 소수로 변환
      networkEfficiency: data.totalRequests > 0 ? 
        (data.totalRequests - data.cacheStats.fromNetwork) / data.totalRequests : 0,
