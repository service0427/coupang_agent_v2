...
class NetworkAnalyzer {
   ...
   * 네트워크 데이터 분석
   */
  analyze(networkData, optimizationContext = null) {
    ...
    // 트래픽 감소 권장사항 생성
    const recommendations = this.generateRecommendations(sortedDomains, sortedResourceTypes, largeResources, optimizationContext);
   ...
   * 트래픽 감소 권장사항 생성
   */
  generateRecommendations(domains, resourceTypes, largeResources, optimizationContext = null) {
    const recommendations = [];
    const { cacheStats } = this.analysisResults || {};

    // 이미지 최적화 권장 (실제 설정 기반 판단)
    const imageStats = resourceTypes.find(r => r.type === 'image');
    if (imageStats && parseFloat(imageStats.percentage) > 30) {
      const isImageBlocked = optimizationContext?.imageBlocked;
      const isOptimizationActive = optimizationContext?.optimizationActive;
      
      // 이미지가 차단 설정되어 있는데도 30% 이상이면 문제
      if (isImageBlocked && isOptimizationActive) {
        recommendations.push({
          type: 'IMAGE_OPTIMIZATION',
          priority: 'HIGH', 
          message: `이미지 차단 설정에도 불구하고 ${imageStats.percentage}%를 차지합니다. 차단 설정을 확인하거나 더 강한 차단을 고려하세요.`,
          savingPotential: `${(imageStats.size / 1024 / 1024).toFixed(2)} MB`
        });
      } 
      // 이미지가 허용 설정되어 있고 30% 이상이면 차단 권장
      else if (!isImageBlocked && isOptimizationActive) {
        recommendations.push({
          type: 'IMAGE_OPTIMIZATION',
          priority: 'HIGH',
          message: `이미지가 전체 트래픽의 ${imageStats.percentage}%를 차지합니다. 이미지 차단을 고려하세요.`,
          savingPotential: `${(imageStats.size / 1024 / 1024).toFixed(2)} MB`
        });
      }
      // 최적화가 비활성화된 경우
        ...
        recommendations.push({
          type: 'IMAGE_OPTIMIZATION',
          priority: 'MEDIUM',
          message: `이미지가 전체 트래픽의 ${imageStats.percentage}%를 차지합니다. 최적화를 활성화하여 이미지 차단을 고려하세요.`,
          savingPotential: `${(imageStats.size / 1024 / 1024).toFixed(2)} MB`
        });
      }
    }
   ...
   * 콘솔에 분석 결과 출력 (요약)
   */
  printAnalysis(keywordId = null, agent = null) {
    ...
    console.log('\n' + '='.repeat(80));
    console.log(`[Traffic Analysis] ${idInfo}Network Traffic Analysis Result (Summary)`);
