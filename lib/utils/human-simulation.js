...
async function naturalBeforeCartClick(page, cartButton) {
  const timeout = new TimeoutContext(CONFIG.TIME_LIMITS.BEFORE_CART_CLICK);
  const viewport = page.viewportSize();

  // 행동 선택 (시간 제한 고려)
  const maxActions = Math.min(3, Math.floor(timeout.maxDuration / 3000));
  const actionCount = randomInt(1, maxActions);
  const sequence = selectWeightedActions(DETAIL_PAGE_ACTIONS, actionCount);
  console.log(`[HumanSim] 상세페이지 행동 (max ${timeout.maxDuration}ms): ${sequence.map(a => a.name).join(' → ')}`);

  for (const action of sequence) {
    if (timeout.expired || timeout.remaining < 1000) {
      log(`⏱️ 시간 부족 - 남은 행동 건너뜀 (${timeout.elapsed}ms 경과)`);
      break;
    }
    ...
    try {
      const actionPromise = (async () => {
        switch (action.name) {
          case 'SCROLL_TO_BOTTOM': {
            ...
            const remaining = pageHeight - currentScroll - viewport.height;
            if (remaining > 50) {
              await naturalScroll(page, Math.min(remaining, 1500), 'down');
          ...
          case 'PAUSE_READING':
            await executePauseReading(page, {
              duration: randomInt(800, 1500),
              microMoveCount: randomInt(2, 4)
            });
            break;
          case 'MOUSE_WANDER':
            await executeMouseWander(page, { moveCount: randomInt(1, 2) });
            break;
          case 'HOVER_IMAGE': {
            const images = await page.$$('.prod-image img, .gallery img, [class*="ProductImage"] img');
            if (images.length > 0) {
              const randomImg = images[randomInt(0, images.length - 1)];
              await naturalMouseMoveToElement(page, randomImg);
              await hoverWithMicroMove(page, randomImg, randomInt(400, 800));
            }
            break;
          }
        }
      ...
      await withTimeout(actionPromise, Math.min(CONFIG.TIME_LIMITS.SINGLE_ACTION, timeout.remaining - 500), action.name);

      if (timeout.remaining > 400) {
        await sleep(Math.min(randomInt(200, 400), timeout.remaining - 300));
      }
    } catch (e) {
      log(`Detail page action ${action.name} error: ${e.message}`);
    }
  }

  // 장바구니 버튼으로 이동 (남은 시간 내에서)
  if (timeout.remaining > 300) {
    try {
      await withTimeout(
        (async () => {
          await scrollToElement(page, cartButton);
          await sleep(Math.min(randomInt(100, 250), timeout.remaining - 200));
          await naturalMouseMoveToElement(page, cartButton);
          await hoverWithMicroMove(page, cartButton, randomInt(200, 400));
        })(),
        timeout.remaining,
        'cart-button-approach'
      );
    } catch (e) {
      log(`Cart button approach error: ${e.message}`);
    }
  }

  console.log(`[HumanSim] 장바구니 클릭 전 행동 완료 (${timeout.elapsed}ms)`);
}
