/**
 * 사람처럼 클릭하는 유틸리티
 * - 랜덤 좌표 클릭
 * - 마우스 다운/업 랜덤 딜레이
 * - 시각적 효과 (하이라이트, 클릭 애니메이션)
 *
 * Updated: 2025-10-09 - 마우스 다운/클릭 지속/업 딜레이 개선
 * Updated: 2025-12-01 - wheel/pointer 이벤트 통합 (human-simulation.js)
 */

const humanSimulation = require('./human-simulation');
const { getTimestamp } = require('./common-helpers');

// 마우스 클릭 타이밍 설정 (Akamai 차단 개선)
const MOUSE_DELAYS = {
  BEFORE_DOWN: { min: 50, max: 150 },    // 마우스 다운 전 대기
  CLICK_HOLD: { min: 30, max: 100 },     // 버튼 누르고 있는 시간
  AFTER_UP: { min: 100, max: 200 }       // 마우스 업 후 대기
};

/**
 * 랜덤 딜레이 헬퍼
 */
function randomDelay(min, max) {
  return min + Math.random() * (max - min);
}

/**
 * 요소의 클릭 가능한 랜덤 좌표 생성
 * - 뷰포트 범위 내에서만 좌표 반환
 * - 요소가 화면 밖이면 스크롤 후 재계산
 * @param {Page} page - Playwright 페이지 객체
 * @param {ElementHandle} element - 클릭할 요소
 */
async function getRandomClickPoint(page, element) {
  // 요소가 보이는지 확인
  const isVisible = await element.isVisible();
  if (!isVisible) {
    throw new Error('요소가 화면에 보이지 않습니다');
  }

  // boundingBox 가져오기 (최대 3번 재시도)
  let box = null;
  for (let i = 0; i < 3; i++) {
    box = await element.boundingBox();
    if (box) break;
    await page.waitForTimeout(500);
  }

  if (!box) {
    // boundingBox 실패 시 요소 정보 디버깅
    const elementInfo = await element.evaluate(el => ({
      tagName: el.tagName,
      className: el.className,
      id: el.id,
      display: window.getComputedStyle(el).display,
      visibility: window.getComputedStyle(el).visibility,
      width: el.offsetWidth,
      height: el.offsetHeight
    }));
    console.error('   요소 정보:', elementInfo);
    throw new Error('요소의 boundingBox를 가져올 수 없습니다');
  }

  // 뷰포트 크기 가져오기
  const viewport = page.viewportSize();

  // 요소가 뷰포트 내에 완전히 보이는지 확인
  const isFullyVisible = box.y >= 0 &&
                         box.y + box.height <= viewport.height &&
                         box.x >= 0 &&
                         box.x + box.width <= viewport.width;

  // 요소가 화면 밖이면 최소한의 스크롤로 화면 안에 들어오게
  if (!isFullyVisible) {
    console.log('   ⚠️ 요소가 화면에 일부만 보임, 스크롤 조정 중...');

    // 필요한 최소 스크롤량 계산
    await element.evaluate((el) => {
      const rect = el.getBoundingClientRect();
      const viewportHeight = window.innerHeight;
      const margin = 50; // 여유 마진

      let scrollAmount = 0;

      if (rect.top < margin) {
        // 요소가 위로 잘림 → 위로 스크롤 (요소가 내려옴)
        scrollAmount = rect.top - margin;
      } else if (rect.bottom > viewportHeight - margin) {
        // 요소가 아래로 잘림 → 아래로 스크롤 (요소가 올라옴)
        scrollAmount = rect.bottom - viewportHeight + margin;
      }

      if (scrollAmount !== 0) {
        window.scrollBy({
          top: scrollAmount,
          behavior: 'instant'
        });
      }
    });

    // 스크롤 후 잠시 대기
    await page.waitForTimeout(200);

    // boundingBox 다시 가져오기
    box = await element.boundingBox();
    if (!box) {
      throw new Error('스크롤 후 요소의 boundingBox를 가져올 수 없습니다');
    }

    console.log(`   ✅ 스크롤 조정 완료 (y: ${Math.round(box.y)})`);
  }

  // 요소 크기의 20~80% 범위 내에서 랜덤 좌표 선택 (가장자리 제외)
  let xMin = box.x + box.width * 0.2;
  let xMax = box.x + box.width * 0.8;
  let yMin = box.y + box.height * 0.2;
  let yMax = box.y + box.height * 0.8;

  // 뷰포트 범위로 클램핑 (안전 마진 10px)
  xMin = Math.max(10, xMin);
  xMax = Math.min(viewport.width - 10, xMax);
  yMin = Math.max(10, yMin);
  yMax = Math.min(viewport.height - 10, yMax);

  // 범위가 유효한지 확인
  if (xMin >= xMax || yMin >= yMax) {
    // 범위가 유효하지 않으면 요소 중심점 사용
    const x = Math.max(10, Math.min(viewport.width - 10, box.x + box.width / 2));
    const y = Math.max(10, Math.min(viewport.height - 10, box.y + box.height / 2));
    console.log(`   ⚠️ 좌표 범위 제한, 중심점 사용 (${Math.round(x)}, ${Math.round(y)})`);
    return { x, y };
  }

  const x = xMin + Math.random() * (xMax - xMin);
  const y = yMin + Math.random() * (yMax - yMin);

  return { x, y };
}

/**
 * 사람처럼 자연스러운 클릭
 */
async function humanClick(page, element, keywordId = null) {
  const idPrefix = keywordId ? `[ID:${keywordId}] ` : '';

  try {
    // v2: 자연스러운 스크롤 (wheel 이벤트 발생)
    await humanSimulation.scrollToElement(page, element);
    await page.waitForTimeout(300 + Math.random() * 200);
    
    // 요소 하이라이트 효과 추가
    await element.evaluate(el => {
      el.style.outline = '3px solid #ff6b6b';
      el.style.outlineOffset = '2px';
      el.style.transition = 'all 0.3s ease';
      el.style.boxShadow = '0 0 20px rgba(255, 107, 107, 0.5)';
    });
    console.log(`   ${idPrefix}🎯 클릭할 요소 하이라이트`);
    await page.waitForTimeout(500); // 하이라이트 효과 보여주기
    
    // 랜덤 클릭 좌표 가져오기
    const { x, y } = await getRandomClickPoint(page, element);

    // v2: 자연스러운 마우스 이동 (pointer/mouse 이벤트 발생)
    await humanSimulation.naturalMouseMove(page, x, y);

    // 클릭 포인트 시각화 (디버그용)
    try {
      await page.evaluate(({clickX, clickY}) => {
        const marker = document.createElement('div');
        marker.style.cssText = `
          position: fixed; left: ${clickX}px; top: ${clickY}px;
          width: 20px; height: 20px; border-radius: 50%;
          background-color: #ff0000; border: 2px solid #ffffff;
          box-shadow: 0 0 10px rgba(255, 0, 0, 0.5);
          pointer-events: none; z-index: 999999;
          transform: translate(-50%, -50%);
        `;
        marker.id = 'click-marker';
        document.body.appendChild(marker);

        const ripple = document.createElement('div');
        ripple.style.cssText = `
          position: fixed; left: ${clickX}px; top: ${clickY}px;
          width: 40px; height: 40px; border-radius: 50%;
          border: 2px solid #ff0000; background-color: transparent;
          pointer-events: none; z-index: 999998;
          transform: translate(-50%, -50%);
          animation: ripple 1s ease-out;
        `;
        ripple.id = 'click-ripple';

        if (!document.getElementById('ripple-style')) {
          const style = document.createElement('style');
          style.id = 'ripple-style';
          style.textContent = `
            @keyframes ripple {
              0% { width: 40px; height: 40px; opacity: 1; }
              100% { width: 100px; height: 100px; opacity: 0; }
            }
          `;
          document.head.appendChild(style);
        }
        document.body.appendChild(ripple);
      }, {clickX: x, clickY: y});
    } catch (e) {
      // 시각화 실패는 무시
    }

    // 이동 후 짧은 대기
    await page.waitForTimeout(randomDelay(80, 150));

    // 마우스 다운 전 짧은 대기 (사람처럼)
    const beforeDownDelay = randomDelay(MOUSE_DELAYS.BEFORE_DOWN.min, MOUSE_DELAYS.BEFORE_DOWN.max);
    await page.waitForTimeout(beforeDownDelay);

    // 마우스 다운
    await page.mouse.down();
    
    // 마우스 다운 시 시각 효과 (네비게이션으로 인한 컨텍스트 파괴 오류 무시)
    try {
      await page.evaluate(() => {
        const marker = document.getElementById('click-marker');
        if (marker) {
          marker.style.backgroundColor = '#ff6600';
          marker.style.transform = 'translate(-50%, -50%) scale(0.8)';
        }
      });
    } catch (error) {
      // 네비게이션으로 인한 컨텍스트 파괴는 정상적인 동작
      if (!error.message.includes('Execution context was destroyed')) {
        console.log(`   ${idPrefix}마우스 다운 시각 효과 중 오류: ${error.message}`);
      }
    }

    // 클릭 지속 시간 (버튼 누르고 있기)
    const holdDelay = randomDelay(MOUSE_DELAYS.CLICK_HOLD.min, MOUSE_DELAYS.CLICK_HOLD.max);
    await page.waitForTimeout(holdDelay);

    // 마우스 업
    await page.mouse.up();
    
    // 마우스 업 시 시각 효과 (네비게이션으로 인한 컨텍스트 파괴 오류 무시)
    try {
      await page.evaluate(() => {
        const marker = document.getElementById('click-marker');
        if (marker) {
          marker.style.backgroundColor = '#00ff00';
          marker.style.transform = 'translate(-50%, -50%) scale(1.2)';
        }
      });
    } catch (error) {
      // 네비게이션으로 인한 컨텍스트 파괴는 정상적인 동작
      if (!error.message.includes('Execution context was destroyed')) {
        console.log(`   ${idPrefix}마우스 업 시각 효과 중 오류: ${error.message}`);
      }
    }

    // 클릭 후 대기 (Akamai 트래킹 요청 완료 대기 포함)
    const afterUpDelay = randomDelay(MOUSE_DELAYS.AFTER_UP.min, MOUSE_DELAYS.AFTER_UP.max);
    await page.waitForTimeout(afterUpDelay);

    // Akamai 이벤트 응답 대기 (click_search_product, impression_ranking 등)
    // 네비게이션 전에 Akamai 요청이 완료되어야 봇으로 판단하지 않음
    console.log(`[${getTimestamp()}] ${idPrefix}⏳ Akamai 이벤트 응답 대기 중...`);
    try {
      // ljc.coupang.com 응답 최대 3개까지 대기 (최대 2초)
      const waitStart = Date.now();
      let receivedCount = 0;
      const maxWait = 2000;

      while (receivedCount < 3 && (Date.now() - waitStart) < maxWait) {
        try {
          await page.waitForResponse(
            res => res.url().includes('ljc.coupang.com'),
            { timeout: 500 }
          );
          receivedCount++;
        } catch {
          // 타임아웃이면 루프 탈출
          break;
        }
      }

      const elapsed = Date.now() - waitStart;
      console.log(`[${getTimestamp()}] ${idPrefix}✅ Akamai 응답 ${receivedCount}개 수신 (${elapsed}ms)`);
    } catch (e) {
      // 응답 대기 실패해도 계속 진행
      console.log(`[${getTimestamp()}] ${idPrefix}⚠️ Akamai 응답 대기 스킵`);
    }
    
    // 시각 효과 제거 (네비게이션으로 인한 컨텍스트 파괴 오류 무시)
    try {
      await page.evaluate(() => {
        const marker = document.getElementById('click-marker');
        const ripple = document.getElementById('click-ripple');
        if (marker) marker.remove();
        if (ripple) ripple.remove();
      });
    } catch (error) {
      // 네비게이션으로 인한 컨텍스트 파괴는 정상적인 동작
      if (!error.message.includes('Execution context was destroyed')) {
        console.log(`   ${idPrefix}시각 효과 제거 중 오류: ${error.message}`);
      }
    }
    
    // 요소 하이라이트 제거 (네비게이션으로 인한 컨텍스트 파괴 오류 무시)
    try {
      await element.evaluate(el => {
        el.style.outline = '';
        el.style.outlineOffset = '';
        el.style.boxShadow = '';
      });
    } catch (error) {
      // 네비게이션으로 인한 컨텍스트 파괴는 정상적인 동작 (무시)
      // - Execution context was destroyed: 페이지 네비게이션
      // - Cannot find context with specified id: DOM 컨텍스트 소실
      const isContextError = error.message.includes('Execution context was destroyed') ||
                             error.message.includes('Cannot find context with specified id');
      if (!isContextError) {
        console.log(`   ${idPrefix}하이라이트 제거 중 오류: ${error.message}`);
      }
    }
    
    console.log(`   ${idPrefix}✅ 사람처럼 클릭 완료`);
    
  } catch (error) {
    console.error(`   ${idPrefix}❌ 사람처럼 클릭 실패:`, error.message);
    throw error;
  }
}

/**
 * 요소의 중심점 좌표 가져오기 (디버깅용)
 */
async function getElementCenter(element) {
  const box = await element.boundingBox();
  if (!box) {
    throw new Error('요소의 boundingBox를 가져올 수 없습니다');
  }
  
  return {
    x: box.x + box.width / 2,
    y: box.y + box.height / 2
  };
}

module.exports = {
  humanClick,
  getRandomClickPoint,
  getElementCenter
};