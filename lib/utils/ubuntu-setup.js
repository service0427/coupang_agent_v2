...
class UbuntuSetup {
  ...
  static async checkSystemResources() {
    ...
    try {
      ...
      if (shmLine) {
        const shmParts = shmLine.split(/\\s+/);
        const size = shmParts[1] || 'unknown';
        const used = shmParts[2] || 'unknown';
        const avail = shmParts[3] || 'unknown';
        
        console.log(`📁 /dev/shm 공간: ${size} (사용중: ${used}, 여유: ${avail})`);
        
        // 여유 공간이 100MB 미만인 경우 경고 (avail이 유효한 경우에만)
        if (avail && avail !== 'unknown') {
          try {
            ...
            console.log('⚠️ /dev/shm 공간 파싱 실패 - 기본 Chrome 인자 적용');
          }
        }
      } else {
        console.log('📁 /dev/shm 공간: 정보를 가져올 수 없음');
      }
