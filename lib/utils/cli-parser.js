...
function parseArgs() {
  ...
  const options = {
    ...
    workType: null,       // 작업 타입 지정 (rank, rank_click, click, product_info 등)
  };
  ...
  for (let i = 0; i < args.length; i++) {
    switch (args[i]) {
      ...
      case '--keep-browser':
        ...
        break;
      case '--work-type':
      case '--work_type':  // 언더스코어 버전도 지원
        options.workType = args[++i];
        break;
    }
  }
...
실행 옵션:
  ...
  --work-type <타입>     작업 타입 지정 (rank, rank_click, click, product_info)
  --work_type <타입>     위와 동일 (언더스코어 버전)
  --help                 도움말 표시
...
예시:
  ...
  # 특정 작업 타입만 실행
  node index.js --work-type rank
  node index.js --work-type rank_click
