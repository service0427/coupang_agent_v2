 ...
 * - 마우스 곡선 이동
 *
 * v2: human-simulation.js 통합 - wheel/pointer 이벤트, 자연스러운 행동 시퀀스
 */

// human-simulation 모듈 로드
...
const DELAYS = {
  // 페이지 로드 후 대기 (빠른 버전)
  THINK_TIME: { min: 800, max: 1500 },       // 생각하는 시간 (페이지 확인)
  AFTER_LOAD: { min: 1000, max: 1500 },      // 로드 후 일반 대기
  BEFORE_CLICK: { min: 100, max: 300 },      // 클릭 전 대기
  BEFORE_TYPE: { min: 200, max: 500 },       // 타이핑 전 대기

  // 타이핑 속도 (50% 빠르게)
  TYPING_PER_CHAR: { min: 40, max: 100 },    // 글자당 딜레이
  TYPING_PAUSE: { min: 150, max: 400 },      // 단어 사이 긴 멈춤 (10% 확률)

  // 마우스/호버
  HOVER_TIME: { min: 300, max: 800 },        // 요소 위에 머무는 시간
  MOUSE_MOVE_DELAY: { min: 50, max: 150 },   // 마우스 이동 사이 딜레이

  // 스크롤
  SCROLL_STEP_DELAY: { min: 30, max: 80 },   // 스크롤 단계 사이
  AFTER_SCROLL: { min: 500, max: 800 },      // 스크롤 후 대기
...
async function naturalTyping(page, selector, text, options = {}) {
  log(`Typing: "${text}" (${text.length}자)`);

  // 10자 이상이면 빠른 타이핑 모드
  const isFastMode = text.length >= 10;
  const delayRange = options.delayRange || (isFastMode
    ? { min: 15, max: 40 }   // 빠른 모드: 15~40ms (50% 빠르게)
    : DELAYS.TYPING_PER_CHAR  // 기본 모드: 40~100ms
  );
  const pauseRange = options.pauseRange || DELAYS.TYPING_PAUSE;
  // 빠른 모드에서는 멈춤 없음, 기본 모드는 10%
  const pauseChance = options.pauseChance || (isFastMode ? 0 : 0.1);

  if (isFastMode) {
    log(`Fast typing mode (${text.length}자 >= 10자)`);
  }
...
/**
 * 상품 클릭 전 자연스러운 행동 (v2: 자연스러운 행동 시퀀스)
 * @param {Page} page - Playwright 페이지 객체
 * @param {ElementHandle} productElement - 상품 요소
 * @param {Array} allProducts - 모든 상품 배열 (선택, v2용)
 * @param {boolean} enableFullBehavior - 전체 행동 시퀀스 활성화 (work_type=click일 때만 true)
 * @returns {Promise<void>}
 */
async function beforeProductClick(page, productElement, allProducts = [], enableFullBehavior = true) {
  log(`Before product click behavior (v2, fullBehavior=${enableFullBehavior})`);

  // v2: 자연스러운 행동 시퀀스 사용
  if (allProducts.length > 0) {
    // 전체 상품 목록이 있으면 자연스러운 시퀀스 실행
    const targetProduct = { element: productElement };
    const products = allProducts.map(p => ({
      element: p.element || p,
      ...p
    }));

    await humanSimulation.naturalBeforeProductClick(page, targetProduct, products, enableFullBehavior);
  } else {
    // 단순 모드 (기존 호환) - 항상 상품을 화면에 배치
    await humanSimulation.simpleBeforeClick(page, productElement);
  }
}
...
module.exports = {
  ...
  // v2: human-simulation 함수들 (직접 접근용)
  naturalBeforeProductClick: humanSimulation.naturalBeforeProductClick,
  naturalBeforeCartClick: humanSimulation.naturalBeforeCartClick,
  naturalScroll: humanSimulation.naturalScroll,
  browseScroll: humanSimulation.browseScroll,
  scrollToElement: humanSimulation.scrollToElement,
  naturalMouseMove: humanSimulation.naturalMouseMove,
  dispatchWheelEvent: humanSimulation.dispatchWheelEvent,
  dispatchPointerEvents: humanSimulation.dispatchPointerEvents,
  resetSession: humanSimulation.resetSession,
