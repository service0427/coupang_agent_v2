...
async function executeSearchMode(page, searchQuery, optimizationLevel, options = {}) {
  console.log(`🌐 쿠팡 메인 페이지 접속 중... (검색창 입력 모드)`);
  ...
  try {
    ...
    console.log(`⏳ 페이지 로딩 안정화를 위해 3초 대기...`);
    await page.waitForTimeout(3000);
    
    // 검색창 찾기 및 입력
    console.log(`🔍 검색창을 찾는 중...`);
    ...
    // 검색어 타이핑
    console.log(`⌨️ 검색어 입력 중: "${searchQuery}"`);
    ...
    try {
      ...
      console.log(`⏳ 검색 결과 페이지 로딩 안정화를 위해 5초 대기...`);
      await page.waitForTimeout(5000);
      
      // URL이 검색 결과 페이지로 변경되었는지 확인
      const currentUrl = page.url();
      if (!currentUrl.includes('/np/search')) {
        console.log(`⚠️ URL이 검색 결과 페이지로 변경되지 않음`);
        console.log(`   현재 URL: ${currentUrl}`);
        console.log(`   검색이 실행되지 않았을 가능성이 있습니다`);
        return { success: false, errorMessage: '검색 페이지 이동 실패' };
      } else {
        console.log(`✅ 검색 결과 페이지로 이동 완료`);
      }
      
      return { success: true };
      
    } catch (navError) {
      console.log(`⚠️ 네비게이션 실패:`, navError.message);
    ...
    console.log(`❌ 검색창 입력 중 오류:`, error.message);
    ...
    // 대체 방법: URL 직접 이동
    console.log(`🔄 대체 방법으로 URL 직접 이동 시도...`);
 ...
 * URL 직접 이동 모드로 검색 실행
 */
async function executeDirectMode(page, searchQuery) {
  console.log(`🌐 쿠팡 검색 페이지 접속 중... (URL 직접 이동 모드)`);
  const encodedQuery = encodeURIComponent(searchQuery);
  ...
  if (randomListSize) {
    ...
    console.log(`   📊 리스트 크기: ${randomListSize}개`);
  }
