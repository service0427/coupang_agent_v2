...
async function executeSearchMode(page, searchQuery, optimizationLevel, options = {}) {
  ...
  try {
    ...
    try {
      ...
      // 프록시 연결 실패 시 즉시 종료
      if (gotoError.message.includes('ERR_PROXY_CONNECTION_FAILED')) {
        console.log(`${idPrefix}🚨 프록시 연결 실패 - 즉시 종료`);
        console.log(`${idPrefix}   에러: ${gotoError.message}`);
        
        // 즉시 프로세스 종료 (재시도 없음)
        console.log(`${idPrefix}❌ 프록시 문제로 실행 중단`);
        process.exit(1);
      }
    ...
    try {
      ...
      if (navError.message.includes('ERR_HTTP2_PROTOCOL_ERROR') || 
        ...
        // V2 에러 로깅 추가
        console.log(`${idPrefix}🔍 [DEBUG] V2 로깅 조건 확인: executionId=${options.executionId}, sessionId=${options.sessionId}`);
        if (options.executionId && options.sessionId) {
          try {
            const errorCode = navError.message.includes('ERR_HTTP2_PROTOCOL_ERROR') ? 
            ...
            try {
              const networkInfo = {
                ...
                actualIp: options.actualIp,
                proxyUsed: options.proxyConfig?.server || null,
              ...
              networkState = JSON.stringify(networkInfo);
            } catch (networkError) {
              console.log(`${idPrefix}⚠️ 네트워크 상태 수집 실패: ${networkError.message}`);
            }
                
            await dbServiceV2.logErrorV2(options.executionId, options.sessionId, {
              actionId: actionLogger?.getCurrentActionDbId(),
              errorLevel: 'error',
              errorCode: errorCode,
              errorMessage: navError.message,
              errorStack: navError.stack,
              actionType: 'search_navigation',
              keyword: searchQuery,
            ...
            console.log(`📋 [V2 Error] 검색 모드 에러 로깅 완료: ${errorCode}`);
          } catch (logError) {
            console.error('🔴 [V2 Error] 검색 모드 에러 로깅 실패:', logError.message);
          }
        }
    ...
    if (options.executionId && options.sessionId) {
      try {
        const errorCode = error.message.includes('ERR_HTTP2_PROTOCOL_ERROR') ? 
          'ERR_HTTP2_PROTOCOL_ERROR' :
          error.message.includes('Timeout') ? 'TIMEOUT_ERROR' : 'SEARCH_INPUT_ERROR';
        ...
        try {
          networkState = JSON.stringify({
            timestamp: new Date().toISOString(),
            errorType: errorCode,
            connectionState: error.message.includes('ERR_HTTP2_PROTOCOL_ERROR') ? 'blocked' : 'timeout',
            actualIp: options.actualIp,
            proxyUsed: options.proxyConfig?.server || null,
            errorContext: 'search_input_failure'
          });
        } catch (networkError) {
          console.log(`${idPrefix}⚠️ 네트워크 상태 수집 실패: ${networkError.message}`);
        }
        ...
        console.log(`📋 [V2 Error] 검색 입력 에러 로깅 완료: ${errorCode}`);
      } catch (logError) {
        console.error('🔴 [V2 Error] 검색 입력 에러 로깅 실패:', logError.message);
      }
    }
    ...
    } catch (fallbackGotoError) {
      // 대체 방법에서도 프록시 연결 실패 시 즉시 종료
      if (fallbackGotoError.message.includes('ERR_PROXY_CONNECTION_FAILED')) {
        console.log(`${idPrefix}🚨 대체 방법에서도 프록시 연결 실패 - 즉시 종료`);
        console.log(`${idPrefix}   에러: ${fallbackGotoError.message}`);
