...
async function searchAndClickProduct(page, options = {}) {
  ...
  try {
    ...
    // IP 확인 및 프록시 오류 감지
    const ipCheckResult = await checkIP(page);
    actualIp = ipCheckResult?.ip || null;
    
    // 프록시 오류 감지 시 즉시 실패 처리
    if (ipCheckResult && !ipCheckResult.success) {
      const proxyErrorMessage = `프록시 오류: ${ipCheckResult.error}`;
      console.log(`❌ ${proxyErrorMessage}`);
      
      result.errorMessage = proxyErrorMessage;
      result.errorType = 'proxy_failure';
      result.executionStatus = ExecutionStatus.ERROR_PROXY; // API 모드에서 사용할 상태
      result.durationMs = Date.now() - startTime;
      
      // V2 로깅: 프록시 실패 기록
      if (executionId && actionLogger) {
        try {
          await actionLogger.completeAction({
            success: false,
            errorMessage: proxyErrorMessage,
            errorType: 'proxy_failure',
            additionalData: {
              detectedIp: ipCheckResult.ip,
              errorType: ipCheckResult.errorType,
              networkState: 'proxy_failure'
            }
          });
          
          await dbServiceV2.completeExecutionV2(executionId, {
            success: false,
            finalStatus: 'proxy_failed',
            errorMessage: proxyErrorMessage,
            errorStep: 'ip_check',
            actualIp: ipCheckResult.ip
          });
          console.log(`📋 [V2 Log] 프록시 실패 기록: ${ipCheckResult.ip}`);
        } catch (logError) {
          console.error('🔴 [V2 Log] 프록시 실패 로깅 오류:', logError.message);
        }
      }
    ...
    console.log(`✅ 프록시 정상 - 외부 IP: ${actualIp}`);
    ...
    for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
      ...
      if (targetProduct) {
        ...
        try {
          ...
          result.productInfo = clickResult.productInfo; // 상품 상세 정보 추가
      ...
      try {
        if (result.success) {
          ...
          if (networkMonitor) {
            ...
            console.log(`✅ [V2 Log] 성공 실행 완료: ${agent} (${finalSearchMode ? 'search' : 'goto'} 모드)`);
          }
  ...
  // 에러가 발생한 경우 에러를 던져서 상위로 전파
  if (result.errorMessage && !result.success) {
    // 차단 관련 에러인 경우 명확한 에러 타입으로 던지기
    if (result.errorMessage.includes('ERR_HTTP2_PROTOCOL_ERROR') || 
      ...
      const blockError = new Error(result.errorMessage);
      blockError.type = 'BLOCKED';
      throw blockError;
    } else {
