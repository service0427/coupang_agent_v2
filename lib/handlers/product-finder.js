  ...
  const products = await page.$$eval(SELECTORS.PRODUCT_LIST.ITEMS, (items, data) => {
    ...
    return items.map((item, index) => {
      ...
      try {
        ...
        if (hasRankParam) {
          urlRank = parseInt(url.searchParams.get('rank')) || null;
        }
      } catch (e) {
        urlParams = href;
      ...
      // 상품명 추출 (개선된 선택자)
      const productNameEl = item.querySelector(productNameSelector) || 
                           item.querySelector('.name, .search-product-name, .prod-name, [data-testid="product-name"]');
      const productName = productNameEl?.textContent?.trim() || '';
      
      // 평점 추출 (여러 선택자 시도)
      const ratingEl = item.querySelector('.rating, .star-rating, .product-rating, [data-testid="rating"], .rating-star, .rating-score');
      let rating = null;
      if (ratingEl) {
        // 텍스트에서 숫자 추출 (예: "4.5", "평점 4.3")
        const ratingText = ratingEl.textContent?.trim() || '';
        const ratingMatch = ratingText.match(/(\d+\.?\d*)/);
        if (ratingMatch) {
          rating = parseFloat(ratingMatch[1]);
        }
        // 별 개수로 평점 추출 (예: 5개 별 중 4개 채워짐)
        if (!rating) {
          const filledStars = ratingEl.querySelectorAll('.filled, .active, .on').length;
          const totalStars = ratingEl.querySelectorAll('.star, .rating-star').length;
          if (filledStars > 0 && totalStars > 0) {
      ...
      // 리뷰수 추출 (여러 선택자 시도)
      const reviewEl = item.querySelector('.review-count, .reviews-count, .product-review-count, [data-testid="review-count"], .rating-total-count');
      let reviewCount = null;
      if (reviewEl) {
        // 텍스트에서 숫자 추출 (예: "(1,234)", "리뷰 1234개", "1234건")
        const reviewText = reviewEl.textContent?.trim() || '';
        const reviewMatch = reviewText.match(/(\d{1,3}(?:,\d{3})*|\d+)/);
        if (reviewMatch) {
      ...
      // 가격 정보 추출 (선택적)
      const priceEl = item.querySelector('.price, .product-price, .sale-price, [data-testid="price"]');
      let price = null;
      if (priceEl) {
...
function findTargetProduct(products, productCode, keywordId = null, actionLogger = null) {
  ...
  if (products.length > 0) {
    const withInfo = products.filter(p => p.productInfo.name && (p.productInfo.rating || p.productInfo.reviewCount)).length;
    if (withInfo > 0) {
      console.log(`   ${idPrefix}상세정보 포함: ${withInfo}개 (제목, 평점, 리뷰수)`);
    }
  }
  ...
  if (!productCode) {
    ...
    if (validProducts.length > 0) {
      ...
      console.log(`   ${idPrefix}상품명: ${targetProduct.productInfo.name}`);
      if (targetProduct.productInfo.rating) {
        console.log(`   ${idPrefix}평점: ${targetProduct.productInfo.rating}점`);
      }
      if (targetProduct.productInfo.reviewCount) {
        console.log(`   ${idPrefix}리뷰: ${targetProduct.productInfo.reviewCount}개`);
      }
    }
  } else {
...
async function clickProduct(page, targetProduct, productCode, pageNum, productsPerPage, keywordId = null, actionLogger = null, executionLogger = null) {
  ...
  console.log(`   ${idPrefix}상품명: ${targetProduct.productInfo.name || targetProduct.productName}`);
  if (targetProduct.productInfo.rating) {
    console.log(`   ${idPrefix}평점: ${targetProduct.productInfo.rating}점`);
  }
  if (targetProduct.productInfo.reviewCount) {
  ...
  return {
    ...
    vendorItemId,
    // 상품 상세 정보 추가 (허브 API 전송용)
    productInfo: targetProduct.productInfo
  };
}
