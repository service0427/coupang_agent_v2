...
const SharedCacheManager = require('../services/shared-cache-manager');
const { cleanChromeProfile } = require('../utils/preferences-cleaner');
// Note: db-service-v2 제거됨 - API 모드에서는 허브 서버가 모든 데이터베이스 작업 처리
...
// normalizeResultData 함수 제거됨 - 직접 필드 설정으로 변경

// ExecutionStatus 관련 import 제거 (간소화)
...
class ApiModeRunner {
  ...
  async executeWork(workAllocation, threadIndex) {
    const threadNumber = threadIndex + 1;
    const hybridProfileManager = this.hybridProfileManagers.get(threadIndex);
    ...
    try {
      ...
      const userFolderPath = `/home/tech/coupang_agent_v1/browser-data/${folderNumber}`;
      ...
      const automationPromise = executeKeywordSearch(
        ...
        keywordData,
        { 
          checkCookies: this.options.checkCookies || false,
          monitor: this.options.monitor || false,
      ...
      if (isSuccess) {
        ...
        if (errorType === 'blocked') {
          this.stats.blocked++;
          console.log(`🚫 [쓰레드 ${threadNumber}] 작업 차단됨: ${executionTime}ms`);
        ...
        } else if (automationResult && !automationResult.productFound) {
          if (errorType === 'blocked') {
            console.log(`   📍 오류: 차단 의심 (referer 검증 실패)`);
          } else {
        ...
        if (refererValidated && errorType === 'blocked') {
          ...
          if (!finalErrorMessage || finalErrorMessage === '상품을 찾을 수 없음') {
            if (actualPageNumber > 0) {
              finalErrorMessage = `페이지 ${actualPageNumber}에서 비정상 종료 (차단 의심)`;
            } else {
              finalErrorMessage = `Referer 없음 - 비정상 종료 (차단 의심)`;
            }
          }
  ...
  getStatus() {
    return {
      baseThreadNumber: this.baseThreadNumber,
      instanceCount: this.threadCount,
      isRunning: this.isRunning,
      stats: this.stats,
      activeThreads: Array.from(this.activeThreads.entries()).map(([index, info]) => ({
        instanceIndex: index,
      ...
      hubApiClients: Array.from(this.hubApiClients.entries()).map(([index, client]) => ({
        instanceIndex: index,
        threadNumber: this.baseThreadNumber + index,
