...
class ApiModeRunner {
  ...
  async executeWork(workAllocation, instanceIndex) {
    ...
    try {
      ...
      // 임시로 간단한 프로필 설정으로 테스트
      const hubFolderNumber = workAllocation.folder;
      const simpleProfilePath = `browser-data/instance${instanceNumber}`;
      
      console.log(`   📁 [인스턴스 ${instanceNumber}] 간단한 프로필 사용`);
      console.log(`   📂 프로필 경로: ${simpleProfilePath}`);
      ...
      console.log(`   🚀 [인스턴스 ${instanceNumber}] 브라우저 실행 중... (세션 독립 + 캐시 공유, GUI 모드)`);
      const browserInfo = await browserManager.getBrowser({
        proxyConfig,
        usePersistent: true,
        profileName: `instance${instanceNumber}`,
        userDataDir: simpleProfilePath, // 간단한 프로필 경로 사용
