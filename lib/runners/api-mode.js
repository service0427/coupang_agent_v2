...
class ApiModeRunner {
  ...
  async executeWork(workAllocation, instanceIndex) {
    ...
    try {
      ...
      // 올바른 유저폴더 구조: instance{N}/{폴더번호}
      const hubFolderNumber = workAllocation.folder;
      const paddedFolderNumber = hubFolderNumber.toString().padStart(3, '0'); // 3자리 패딩
      const userFolderPath = `browser-data/instance${instanceNumber}/${paddedFolderNumber}`;
      
      console.log(`   📁 [인스턴스 ${instanceNumber}] 허브 폴더 ${paddedFolderNumber} 사용`);
      ...
      // 브라우저 실행 (GUI 모드 유지, Ubuntu 최적화 인자는 자동 적용)
      console.log(`   🚀 [인스턴스 ${instanceNumber}] 브라우저 실행 중... (폴더별 독립 실행, GUI 모드)`);
      const browserInfo = await browserManager.getBrowser({
        proxyConfig,
        usePersistent: true,
        profileName: `instance${instanceNumber}_${paddedFolderNumber}`,
        userDataDir: userFolderPath, // 올바른 유저폴더 경로 사용
        clearSession: false, // 세션 유지
        clearCache: false,   // 캐시 유지 (공유 없이 독립)
        headless: false,     // GUI 모드 유지 (Ubuntu GUI 환경에서도 직접 구동)
      ...
      // 상세한 실행 결과 로그 출력
      if (isHttpSuccessResult) {
        console.log(`✅ [인스턴스 ${instanceNumber}] 작업 성공 완료: ${executionTime}ms (${statusCode} - ${statusInfo.status})`);
      } else {
        console.log(`❌ [인스턴스 ${instanceNumber}] 작업 실패: ${executionTime}ms (${statusCode} - ${statusInfo.status})`);
        console.log(`   📍 실행 상태: ${executionStatus}`);
        if (errorMessage) {
          console.log(`   📍 오류 메시지: ${errorMessage}`);
        }
        if (automationResult?.successLevel) {
          console.log(`   📍 성공 단계: ${automationResult.successLevel}`);
        }
        if (isBlockedResult && automationResult?.blockedReason) {
          console.log(`   📍 차단 사유: ${automationResult.blockedReason}`);
        }
        if (executionStatus === ExecutionStatus.ERROR_TIMEOUT && automationResult?.timeoutReason) {
          console.log(`   📍 타임아웃 사유: ${automationResult.timeoutReason}`);
        }
      }
      ...
      // 성능 메트릭 수집
      const performanceData = await this.collectPerformanceMetrics(automationResult, networkMonitor);
      ...
      return {
        ...
        result: {
          ...
          errorDetails: automationResult?.errorDetails || null,
          successLevel: automationResult?.successLevel || null,
          blockedReason: isBlockedResult ? (automationResult?.blockedReason || 'Unknown blocked reason') : null,
          timeoutReason: executionStatus === ExecutionStatus.ERROR_TIMEOUT ? (automationResult?.timeoutReason || 'Operation timeout') : null
        },
      ...
      // Ubuntu GUI 환경에서 발생하는 브라우저 실행 오류 분석
      let detailedError = error.message;
      let ubuntuTroubleshooting = '';
      
      if (process.platform === 'linux') {
        console.log(`🐧 [Ubuntu 디버깅] 전체 오류 스택:`);
        console.log(error.stack || '스택 정보 없음');
        
        if (error.message.includes('No usable sandbox') || 
            error.message.includes('Failed to move to new namespace')) {
          ubuntuTroubleshooting = '\\n💡 해결방법: Chrome 샌드박스 권한 문제입니다. --no-sandbox 인자가 적용되었지만 여전히 문제가 있을 수 있습니다.';
        } else if (error.message.includes('/dev/shm')) {
          ubuntuTroubleshooting = '\\n💡 해결방법: /dev/shm 공간 부족입니다. --disable-dev-shm-usage 인자가 적용되었습니다.';
        } else if (error.message.includes('GPU')) {
          ubuntuTroubleshooting = '\\n💡 해결방법: GPU 관련 문제입니다. 추가 GPU 비활성화 인자를 적용해보세요.';
        } else if (error.message.includes('timeout') || error.message.includes('TimeoutError')) {
          ubuntuTroubleshooting = '\\n💡 해결방법: 브라우저 실행 타임아웃입니다. 시스템 리소스를 확인해보세요.';
        } else if (error.message.includes('ECONNREFUSED') || error.message.includes('connection')) {
          ubuntuTroubleshooting = '\\n💡 해결방법: 브라우저-Playwright 연결 실패입니다. 포트 충돌을 확인해보세요.';
        } else {
          ubuntuTroubleshooting = '\\n🔍 일반적인 Ubuntu Chrome 문제 해결방법:\\n' +
            '   1. Chrome 브라우저 설치: sudo apt install google-chrome-stable\\n' +
            '   2. 종속성 설치: sudo apt install libnss3 libgconf-2-4 libxss1 libasound2\\n' +
            '   3. 권한 확인: Chrome 실행 권한 및 프로필 디렉토리 권한\\n' +
            '   4. 메모리 확인: free -h 명령으로 여유 메모리 확인';
        }
        
        detailedError += ubuntuTroubleshooting;
      }
      ...
      console.error(`❌ [인스턴스 ${instanceNumber}] 작업 실행 실패: ${detailedError} (${errorStatusCode})`);
      this.stats.failed++;
      ...
      return {
        ...
        result: {
          ...
          productsFound: 0,
          // 브라우저 실행 오류 상세 정보 추가
          executionStatus: ExecutionStatus.ERROR_BROWSER,
          errorMessage: detailedError,
          errorDetails: {
            platform: process.platform,
            originalError: error.message,
            troubleshooting: ubuntuTroubleshooting || null,
