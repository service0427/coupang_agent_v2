...
class ApiModeRunner {
  ...
  async executeWork(workAllocation, instanceIndex) {
    ...
    try {
      ...
      // 브라우저 실행 (Ubuntu 환경 감지하여 headless 모드 자동 설정)
      const isUbuntu = process.platform === 'linux' && !process.env.DISPLAY;
      const headlessMode = isUbuntu || process.env.NODE_ENV === 'production';
      
      console.log(`   🚀 [인스턴스 ${instanceNumber}] 브라우저 실행 중... (세션 독립 + 캐시 공유, headless: ${headlessMode})`);
      const browserInfo = await browserManager.getBrowser({
        ...
        headless: headlessMode, // Ubuntu/production 환경에서 자동으로 headless 모드
        windowPosition: headlessMode ? undefined : windowPosition // headless에서는 window position 불필요
      });
      ...
      // Ubuntu 환경에서 발생하는 특정 오류 분석
      let detailedError = error.message;
      if (process.platform === 'linux') {
        if (error.message.includes('No usable sandbox!') || 
            error.message.includes('Failed to move to new namespace')) {
          detailedError += ' (Ubuntu 환경: --no-sandbox 인자가 필요할 수 있습니다)';
        } else if (error.message.includes('/dev/shm')) {
          detailedError += ' (Ubuntu 환경: /dev/shm 공간 부족, --disable-dev-shm-usage 인자가 필요할 수 있습니다)';
        } else if (error.message.includes('DISPLAY')) {
          detailedError += ' (Ubuntu 환경: GUI 디스플레이 없음, headless 모드로 실행됩니다)';
        }
      }
