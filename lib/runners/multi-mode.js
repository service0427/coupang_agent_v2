...
async function runSingleTask(keyword, options, instanceIndex) {
  ...
  try {
    console.log(`\n🔄 [작업 ID:${keyword.id}] 시작 - "${keyword.keyword}" ${keyword.suffix || ''}`);
    ...
    if (result.success) {
      console.log(`✅ [작업 ID:${keyword.id}] 완료 - ${duration}초`);
      if (result.cartClickCount > 0) {
        console.log(`   [ID:${keyword.id}] 🛒 장바구니 클릭: ${result.cartClickCount}회`);
      }
    } else {
      console.log(`❌ [작업 ID:${keyword.id}] 실패 - ${duration}초 - ${result.errorMessage}`);
    }
    
    // 개별 대기 없이 바로 브라우저 종료
    
  } catch (error) {
    console.error(`❌ [작업 ID:${keyword.id}] 오류:`, error.message);
    ...
    while (roundCount < options.maxRounds || !options.once) {
      ...
      results.forEach(result => {
        const status = result.success ? '✅ 성공' : 
                      (result.errorMessage && result.errorMessage.includes('ERR_HTTP2_PROTOCOL_ERROR')) ? '🚫 차단' : '❌ 실패';
        
        if (result.success) successCount++;
        else if (result.errorMessage && result.errorMessage.includes('ERR_HTTP2_PROTOCOL_ERROR')) blockedCount++;
        else failCount++;
