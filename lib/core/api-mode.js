  ...
  async executeWork(workAllocation, threadIndex) {
    ...
    try {
      ...
      if (this.options.chromeVersion) {
        const version = this.options.chromeVersion;
        const fs = require('fs');
        ...
        if (!fs.existsSync(chromeBaseDir)) {
          ...
          console.log(`   ────────────────────────────────────────────`);
          console.log(`   설치 방법:`);
          console.log(`     ./install-chrome-version.sh ${version}`);
          console.log(`   ────────────────────────────────────────────`);
          console.log(`   모든 버전 설치:`);
          console.log(`     ./install-chrome-version.sh all`);
          console.log(`   ────────────────────────────────────────────`);
        ...
        const dirs = fs.readdirSync(chromeBaseDir).filter(dir => {
          // 짧은 버전 (예: 138) 또는 전체 버전 (예: 138.0.7204.49) 매칭
          return dir.startsWith(`chrome-${version.replace(/\./g, '-')}`) ||
                 dir.startsWith(`chrome-${version}`);
        });

        if (dirs.length > 0) {
          // 가장 최신 버전 선택 (정렬된 마지막 항목)
          const selectedDir = dirs.sort().pop();
          ...
          if (fs.existsSync(executablePath)) {
            ...
            if (fs.existsSync(versionFile)) {
              ...
              console.log(`   🎯 [쓰레드 ${threadNumber}] Chrome ${actualChromeVersion} 사용: ${selectedDir}`);
            } else {
          ...
          console.log(`     ./install-chrome-version.sh list`);
          console.log(`   ────────────────────────────────────────────`);
          throw new Error(`Chrome ${version} 버전이 설치되지 않음. 위 명령으로 설치 후 재실행하세요.`);
        }
      }
      ...
      if (!actualChromeVersion || actualChromeVersion === null) {
        try {
          // 방법 1: chrome://version 페이지에서 전체 버전 가져오기 (빌드 번호 포함)
          const versionPage = await browser.newPage();
          try {
            await versionPage.goto('chrome://version', { waitUntil: 'domcontentloaded', timeout: 5000 });

            // 전체 버전 번호 추출
            const fullVersion = await versionPage.evaluate(() => {
              // Chrome 버전 정보가 있는 요소 찾기
              const versionElement = document.getElementById('version');
              if (versionElement) {
                // "140.0.7339.185 (공식 빌드) (64비트)" 형식에서 버전만 추출
            ...
            if (fullVersion) {
              actualChromeVersion = fullVersion;
              console.log(`   📱 [쓰레드 ${threadNumber}] 감지된 Chrome 버전 (전체): ${actualChromeVersion}`);
            }
          } catch (versionError) {
            // chrome://version 접근 실패 시 대체 방법
          } finally {
            await versionPage.close();
          }

          // 방법 2: 전체 버전을 못 가져왔으면 기본 방식 사용
          if (!actualChromeVersion) {
            ...
            if (versionMatch) {
              ...
              // userAgent 방식으로 다시 시도
              const userAgent = await page.evaluate(() => navigator.userAgent);
              const chromeMatch = userAgent.match(/Chrome\/([0-9.]+)/);
              if (chromeMatch) {
                actualChromeVersion = chromeMatch[1];
                console.log(`   📱 [쓰레드 ${threadNumber}] 감지된 Chrome 버전: ${actualChromeVersion}`);
              }
            }
          }
