...
class ApiModeRunner {
  ...
  collectProductData(automationResult) {
    ...
    // targetProduct에서 추가 데이터 가져오기 (product-finder.js에서 수집한 데이터)
    const target = automationResult.targetProduct || {};

    const productData = {
      real_rank: automationResult.realRank ?? null,
      url_rank: automationResult.urlRank ?? null,
      product_code: target.productCode || '',
      product_name: automationResult.productInfo?.name || '',
      rating: automationResult.productInfo?.rating ?? 0,
      review_count: automationResult.productInfo?.reviewCount ?? 0,
      thumbnail_url: automationResult.productInfo?.thumbnailUrl || null,
      href: target.href || '',
      sale_price: target.salePrice ?? 0,
      delivery_type: target.deliveryType ?? null
    };
