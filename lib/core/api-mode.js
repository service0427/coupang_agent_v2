...
class ApiModeRunner {
  ...
  async executeWork(workAllocation, threadIndex) {
    ...
    try {
      ...
      const cleanup = browserInfo.cleanup;
      ...
      if (finalSuccess) {
        ...
        this.statusServer.updateThread(threadNumber, { status: 'idle', keyword: '-', page: '-', proxy: proxyHost, chrome: actualChromeVersion?.split('.')[0] || '?' });
      ...
      this.stats.failed++;
      this.statusServer.updateStats('failed');
      this.statusServer.updateThread(threadNumber, { status: 'error', keyword: workAllocation.work.keyword, page: '-', proxy: '-', chrome: '-' });
      ...
      if (browser && browser.isConnected()) {
        try {
          ...
          // /tmp 임시파일 정리 (5번 작업마다)
          if (this.threadStats[threadIndex].total % 5 === 0) {
            await browserManager.cleanTempFiles();
          }

          // 캐시 크기 관리 (50번 작업마다)
          if (this.threadStats[threadIndex].total % 50 === 0) {
