...
const { getRandomChromeVersion, CHROME_VERSIONS_DIR } = require('./api/chrome-manager');
...
// 세션 재사용 파일 경로
const SESSION_REUSE_FILE = './browser-data/session-reuse.json';
...
class ApiModeRunner {
  ...
  async executeWork(workAllocation, threadIndex) {
    ...
    // 상태 서버 업데이트 - 작업 시작
    const proxyHost = workAllocation.proxy.url ? new URL(workAllocation.proxy.url).hostname : '-';
    ...
    try {
      ...
      // 프로필 경로: browser-data/쓰레드번호/Chrome메이저버전
  ...
  loadSessionReuse() {
    try {
      if (fs.existsSync(SESSION_REUSE_FILE)) {
        const data = fs.readFileSync(SESSION_REUSE_FILE, 'utf-8');
        const parsed = JSON.parse(data);
        console.log(`📂 세션 재사용 정보 로드: ${Object.keys(parsed).length}개 프로필`);
  ...
  saveSessionReuse() {
    try {
      // browser-data 폴더 확인
      const dir = path.dirname(SESSION_REUSE_FILE);
      if (!fs.existsSync(dir)) {
        fs.mkdirSync(dir, { recursive: true });
      }

      const data = Object.fromEntries(this.sessionReuse);
      fs.writeFileSync(SESSION_REUSE_FILE, JSON.stringify(data, null, 2));
    } catch (error) {
