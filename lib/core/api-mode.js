...
function collectInstalledChromeVersions() {
  ...
  const chromeBaseDir = path.join(require('os').homedir(), 'chrome-versions');
...
function getRandomChromeVersion() {
  ...
  const selected = availableChromeVersions[Math.floor(Math.random() * availableChromeVersions.length)];
  return selected;
}
      ...
      if (keywordData.work_type === 'product_info') {
        ...
        automationPromise = executeKeywordSearch(
          ...
          keywordData,
          {
            ...
            // monitor 옵션 제거됨
            threadNumber: threadNumber,  // 쓰레드 번호 추가
            directUrl: this.options.directUrl || false  // URL 직접 모드 옵션 전달
          }
        );
      }
