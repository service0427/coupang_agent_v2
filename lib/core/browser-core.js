...
class BrowserCore {
  ...
  getChromeArgs(options = {}) {
    ...
    const args = [
      ...
      '--no-first-run',
      // WebRTC IP leak 방지 (SOCKS5 프록시 사용 시 실제 IP 노출 차단)
      '--disable-features=WebRtcHideLocalIpsWithMdns,WebRtcIPHandlingPolicy',
      '--force-webrtc-ip-handling-policy=disable_non_proxied_udp',
      '--webrtc-ip-handling-policy=disable_non_proxied_udp',
      // DNS leak 방지 (SOCKS5 프록시가 DNS도 처리하도록)
