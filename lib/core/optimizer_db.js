 ...
 * @param {string} agent - 에이전트 이름 (DB 설정 조회용)
 * @returns {Function} 최적화 해제 함수
 */
async function applyDynamicOptimization(page, agent = null) {
  console.log('🔍 [OptimizerDB] DB 기반 동적 최적화 시작');
  ...
  if (agent) {
    try {
      agentConfig = await dbService.getAgentConfig(agent);
      if (agentConfig) {
        console.log(`🔧 [OptimizerDB] 에이전트 ${agent} 설정 로드 완료 (${agentConfig.test_name})`);
        DOMAIN_RULES_DYNAMIC = buildDomainRulesFromConfig(agentConfig);
      } else {
        console.log(`⚠️  [OptimizerDB] 에이전트 ${agent} 설정 없음, 기본값 사용`);
        DOMAIN_RULES_DYNAMIC = buildDomainRulesFromConfig(null);
      }
    } catch (error) {
      console.error(`❌ [OptimizerDB] DB 설정 로드 실패 (${agent}):`, error.message);
      DOMAIN_RULES_DYNAMIC = buildDomainRulesFromConfig(null);
    }
  } else {
  ...
  // 적용된 설정 요약 출력
  if (agentConfig) {
    console.log(`   🎯 테스트 설정: ${agentConfig.test_name}`);
    console.log(`   📝 www.coupang.com: ${agentConfig.coupang_main_allow || 'DEFAULT'}`);
    console.log(`   📝 front CDN: ${agentConfig.front_cdn_allow || 'DEFAULT'}`);
    if (agentConfig.coupang_main_block_patterns) {
      console.log(`   🚫 blockPatterns: ${agentConfig.coupang_main_block_patterns}`);
    }
  }
