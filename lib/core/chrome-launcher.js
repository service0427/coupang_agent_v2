 ...
 */
async function launchChrome(proxy = null, persistent = false, profileName = null, clearSession = false, clearCache = false, useTracker = false, gpuDisabled = false, windowPosition = null, trafficMonitor = false, customUserDataDir = null) {
  let browser;
  let page;
  let context;
  ...
  if (persistent) {
    ...
    // clearCache가 true일 때만 디렉토리 완전 삭제
    // clearSession만 true면 CDP로 세션만 제거, 캐시는 보존
    if (clearCache) {
      await removeDirectory(userDataDir);
      console.log(`🗑️ 캐시 삭제 설정으로 유저데이터 완전 삭제`);
    } else if (clearSession) {
      console.log('🧹 세션만 제거 예정 (캐시 보존을 위해 유저데이터 유지)');
    } else {
    ...
    // 캐시나 세션을 유지하기 위해 유저데이터를 보존할 때는 persistent context 사용
    if (!clearCache) {
      try {
        await fs.mkdir(userDataDir, { recursive: true });
      } catch (e) {
