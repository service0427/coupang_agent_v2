 ...
 */
async function launchChrome(proxy = null, persistent = false, profileName = null, clearSession = false, useTracker = false, gpuDisabled = false, windowPosition = null) {
  let browser;
  let page;
  let context;
  ...
  // clearSession 옵션 처리 (CDP 방식)
  if (clearSession) {
    await clearSessionWithCDP(page);
  } else if (persistent) {
