...
/**
 * Chrome 버전이 131 이상인지 확인
 * @param {string} chromeVersion - Chrome 버전 (예: "131.0.6778.85")
 * @returns {boolean} 131 이상이면 true
 */
function isChromeVersion131OrHigher(chromeVersion) {
  if (!chromeVersion || chromeVersion === 'default') {
    return false;
  }

  try {
    const majorVersion = parseInt(chromeVersion.split('.')[0]);
    return majorVersion >= 131;
  } catch (error) {
    return false;
  }
}
...
function buildSuccessResponse(workType, allocationKey, proxyId, automationResult, chromeVersion) {
  if (workType === 'product_info') {
    ...
    // Chrome 131 이상일 때만 cookies 추가 (cffi에서 이하 버전은 효과 없음)
    if (isChromeVersion131OrHigher(chromeVersion)) {
