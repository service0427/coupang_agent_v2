...
function buildErrorMessage(automationResult, errorMessage, actualPageNumber, expectedMaxPages) {
  ...
  if (errorMessage && errorMessage.includes('HTTP2_PROTOCOL_ERROR')) {
    ...
    // referer 없음 = 검색 결과 페이지에 도달하지 못함
    // 현재는 메인 페이지 검색 모드만 사용하므로 단일 메시지
    finalErrorMessage = `쿠팡 차단 - 초기 검색 차단`;
  } else {
...
function buildErrorResponse(workType, allocationKey, proxyId, automationResult, finalErrorMessage, chromeVersion) {
  if (workType === 'product_info') {
    ...
    // _abck 쿠키가 있고, 1페이지 차단이 아닌 경우에만 cookies 추가
    // search_blocked_1page, initial_blocked는 쿠키가 쓸모없으므로 제외
    const cookieState = automationResult?.cookieState || 'initial_blocked';
    const isUselessCookie = cookieState === 'search_blocked_1page' || cookieState === 'initial_blocked';

    if (automationResult?.cookies && !isUselessCookie) {
      response.cookies = automationResult.cookies;
      response.cookie_state = cookieState;
    }
...
function buildGeneralErrorResponse(workType, allocationKey, proxyId, error, chromeVersion) {
  if (workType === 'product_info') {
    ...
    // 에러 타입 판별 및 메시지 간소화
    if (error.errorType === 'login_required' || error.message.includes('로그인 필요')) {
      errorType = 'login_required';
      shortMessage = '로그인 필요 - 성인인증 상품';
    } else if (error.message.includes('launchPersistentContext') || error.message.includes('Target page, context or browser has been closed')) {
      errorType = 'browser_error';
      shortMessage = '브라우저 시작 실패';
    } else if (error.message.includes('browser') || error.message.includes('Chrome')) {
      errorType = 'browser_error';
      shortMessage = '브라우저 오류';
      ...
      // 알 수 없는 에러는 첫 줄만 사용 (최대 100자)
      shortMessage = error.message.split('\n')[0].substring(0, 100);
    }
