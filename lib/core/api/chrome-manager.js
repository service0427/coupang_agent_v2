...
// 최소 Chrome 버전 제한 (131 미만 차단됨)
const MIN_CHROME_VERSION = 131;

// Chrome 버전 디렉토리 (install-chrome-deb.sh와 동일 경로)
const CHROME_VERSIONS_DIR = '/home/tech/chrome-versions';
...
function getRandomChromeVersion(excludedBuilds = []) {
  ...
  // 최소 버전 필터링 (131 이상만)
  let filteredVersions = availableChromeVersions.filter(chromeInfo => {
    const major = parseInt(chromeInfo.majorVersion, 10);
    return major >= MIN_CHROME_VERSION;
  });
  console.log(`🔒 Chrome ${MIN_CHROME_VERSION}+ 필터: ${filteredVersions.length}개 (전체: ${availableChromeVersions.length}개)`);

  // 제외할 빌드 필터링
  if (excludedBuilds && excludedBuilds.length > 0) {
    filteredVersions = filteredVersions.filter(chromeInfo => {
      return !excludedBuilds.includes(chromeInfo.version);
    });

    console.log(`🚫 제외된 Chrome 빌드: ${excludedBuilds.length}개`);
    console.log(`✅ 사용 가능한 Chrome 빌드: ${filteredVersions.length}개`);
  }
