 ...
 */
function isRequestAllowed(url, resourceType, monitor = false) {
  let domain = '';
  try {
  ...
  if (!isDomainAllowed) {
    return { allowed: false, silent: !monitor };
  }

  // 2단계: 차단 설정 규칙 매칭 (고정 차단 도메인들)
  const rule = findMatchingRule(domain, BLOCKED_DOMAINS);
  if (!rule) {
    // 규칙이 없는 도메인은 기본 허용 (--monitor 옵션에 따라 로그 제어)
    return { allowed: true, silent: !monitor };
  }

  // 3단계: 차단 여부 확인 (항상 차단)
  const allowed = !rule.blocked;

  // --monitor 옵션이 활성화된 경우에만 로그 표시
  return { allowed, silent: !monitor };
}
...
async function applyStaticOptimization(page, agent = null, keywordData = null, options = {}) {
  ...
  await page.route('**/*', async (route) => {
    ...
    const { allowed, silent } = isRequestAllowed(url, resourceType, options.monitor);
