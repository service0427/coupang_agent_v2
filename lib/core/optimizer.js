      ...
      try {
        ...
        if (postData) {
          ...
          if (eventName === 'click_search_product') {
            // 세션별 memberSrl 생성 (한 번만)
            if (!sessionMemberSrl) {
              sessionMemberSrl = Math.floor(Math.random() * 900000000) + 100000000;
            }

            // memberSrl이 없거나 빈 값이면 주입
            if (!jsonData.common?.memberSrl) {
              if (!jsonData.common) jsonData.common = {};
              jsonData.common.memberSrl = String(sessionMemberSrl);

              const modifiedPostData = JSON.stringify(jsonData);
              pendingLjcRequests.set(request, { eventName, memberSrl: String(sessionMemberSrl) });

              // 세션에 memberSrl 저장 (API 전송용)
              const session = sessionEventsMap.get(allocationKey);
              if (session) {
                session.member_srl = String(sessionMemberSrl);
              }

              console.log(`[${getTimestamp()}] 📡 [Akamai] 🎯 ${eventName} | memberSrl: ${sessionMemberSrl} (주입됨)`);
