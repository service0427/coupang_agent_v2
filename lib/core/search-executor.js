...
async function executeSearch(page, keywordData, options, finalSearchMode, optimizationContext = null, threadPrefix = '') {
  try {
    // 검색 옵션 구성
    // work_type에 'click'이 포함되면 클릭 활성화 (rank_click, click)
    // 포함되지 않으면 클릭 비활성화 (rank, product_info)
    const shouldEnableClick = keywordData.work_type && keywordData.work_type.includes('click');
    
    if (threadPrefix) {
      console.log(`${threadPrefix}📋 work_type: ${keywordData.work_type || 'null'}, 장바구니 클릭: ${shouldEnableClick ? '활성화' : '비활성화'}`);
    }
    ...
    const searchOptions = {
      ...
      maxPages: 10,  // 15페이지는 너무 길어서 10페이지로 제한
