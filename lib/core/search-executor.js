...
async function executeKeywordSearch(page, keywordData, options, networkMonitor = null) {
  ...
  try {
    ...
    // 최적화 적용
    disableOptimization = await applyOptimization(page, keywordData, options);
 ...
 */
async function applyOptimization(page, keywordData, options = {}) {
  console.log('\n🚀 V2 최적화 활성화 (목표: 500KB 이하)');
  console.log(`   필수 도메인만 허용 | 모든 정적 리소스 차단`);
  
  // monitor 옵션을 applyDynamicOptimization에 전달
  const optimizationOptions = {
    monitor: options.monitor || false
  };
  
  // V2 키워드 데이터를 직접 전달하여 키워드별 optimize 설정 사용
  return await applyDynamicOptimization(page, keywordData.agent, keywordData, optimizationOptions);
}
