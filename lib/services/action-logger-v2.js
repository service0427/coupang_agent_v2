...
class ActionLoggerV2 {
  ...
  async updateActionStatus(actionId, newStatus, data = {}, forceTransition = false) {
    ...
    // undefined 상태 체크 강화
    if (!newStatus || newStatus === 'undefined' || typeof newStatus !== 'string') {
      console.warn(`⚠️  잘못된 상태 값: ${newStatus} (type: ${typeof newStatus}, actionId: ${actionId})`);
      console.warn(`   호출 스택:`, new Error().stack?.split('\n').slice(1, 4).join('\n'));
      return false;
    }
