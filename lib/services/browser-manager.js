...
class BrowserManager {
  ...
  async getBrowser(options = {}) {
    ...
    if (usePersistent) {
      ...
      await cleanChromeProfile(actualUserDataDir);
    ...
    try {
      ...
      if (os.platform() === 'win32') {
        ...
        try {
          ...
          // Google Chrome과 Chromium 브라우저 둘 다 처리
          await execAsync(`pkill -f "google-chrome.*${absolutePath}" 2>/dev/null || true`);
          console.log(`   ✅ 자동화용 Chrome 프로세스 정리 완료 (Ubuntu)`);
        } catch (linuxError) {
          // pkill이 프로세스를 찾지 못한 경우는 정상 (이미 종료됨)
          // 불필요한 오류 메시지 표시하지 않음
        }
      } else {
