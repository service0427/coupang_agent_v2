...
class ProxyToggleService {
   ...
   * @param {number} retryDelay - 재시도 간격 (밀리초, 기본값: 30000)
   * @returns {Object} 토글 결과
   */
  async toggleIp(proxyServer, maxRetries = 5, retryDelay = 30000) {
    // 포트 번호 추출
   ...
   */
  async toggleMultiple(proxyServers, maxRetries = 5, retryDelay = 30000) {
    const results = [];
