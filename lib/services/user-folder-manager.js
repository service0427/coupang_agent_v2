/**
 * 유저 폴더 매니저
 * 인스턴스별로 1-30 번호의 유저 폴더를 순차적으로 관리
 */
...
class UserFolderManager {
  constructor(config = {}) {
    ...
    console.log(`📂 UserFolderManager 초기화: 인스턴스 ${this.instanceNumber}`);
  ...
  getFolderPath(folderNumber) {
    return path.join(this.basePath, `instance_${this.instanceNumber}`, `${folderNumber.toString().padStart(3, '0')}`);
  }
