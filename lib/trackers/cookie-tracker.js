...
class CookieTracker {
  ...
  async saveInitialCookies(pageOrContext) {
    try {
      // page 또는 context 모두 지원
      const cookies = pageOrContext.context ? 
        await pageOrContext.context().cookies() : 
        await pageOrContext.cookies();
  ...
  async saveFinalCookies(pageOrContext) {
    try {
      // page 또는 context 모두 지원
      const cookies = pageOrContext.context ? 
        await pageOrContext.context().cookies() : 
