...
class HubApiClient {
  constructor(config = {}) {
    this.hubBaseUrl = config.hubBaseUrl || 'http://mkt.techb.kr:3302';
    this.threadNumber = config.threadNumber || 1;
    this.timeout = config.timeout || 30000;
    this.retryCount = config.retryCount || 3;
    this.workType = config.workType || null;  // work_type 파라미터 추가
  ...
  async allocateWork() {
    ...
    // work_type이 지정된 경우 추가
    if (this.workType) {
      params.append('work_type', this.workType);
      console.log(`   작업 타입: ${this.workType}`);
    }
