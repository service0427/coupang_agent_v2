...
const { isAkamaiEventsSuccess, resetAkamaiEvents } = require('../../core/optimizer');
 ...
 */
async function clickProduct(page, targetProduct, productCode, pageNum, productsPerPage, keywordId = null, threadPrefix = '', allProducts = [], workType = null, allocationKey = null, maxAkamaiRetries = 1) {
  const idPrefix = createIdPrefix(keywordId);
          ...
          if (element) {
            ...
            if (allProducts.length > 0 && enableFullBehavior) {
              ...
              for (const p of allProducts.slice(0, 10)) { // 최대 10개만 (성능)
                try {
                  const selector = `#product-list > li[data-id="${p.productCode}"] a[href*="/vp/products/"]`;
  ...
  // Akamai 이벤트 성공 여부 확인 및 재시도 (click 타입만)
  if (workType === 'click' && allocationKey && maxAkamaiRetries > 0) {
    // 잠시 대기 후 Akamai 이벤트 결과 확인
    await page.waitForTimeout(500);

    if (!isAkamaiEventsSuccess(allocationKey)) {
      console.log(`${threadPrefix}    ${idPrefix}⚠️ Akamai 이벤트 실패 감지 - 뒤로가기 후 재클릭 시도`);

      // 뒤로가기
      try {
        await page.goBack({ waitUntil: 'domcontentloaded', timeout: 10000 });
        console.log(`${threadPrefix}    ${idPrefix}🔙 뒤로가기 완료`);
        await page.waitForTimeout(2000);

        // Akamai 이벤트 초기화 (retry_count 증가)
        resetAkamaiEvents(allocationKey);

        // 재귀 호출로 재클릭 시도 (maxAkamaiRetries - 1)
        console.log(`${threadPrefix}    ${idPrefix}🔄 재클릭 시도 (남은 재시도: ${maxAkamaiRetries - 1})`);
        return await clickProduct(
          page, targetProduct, productCode, pageNum, productsPerPage,
          keywordId, threadPrefix, allProducts, workType,
          allocationKey, maxAkamaiRetries - 1
        );
      } catch (backError) {
        console.log(`${threadPrefix}    ${idPrefix}⚠️ 뒤로가기 실패: ${backError.message} - 계속 진행`);
      }
    } else {
      console.log(`${threadPrefix}    ${idPrefix}✅ Akamai 이벤트 성공`);
    }
  }
