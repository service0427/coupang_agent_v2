    ...
    for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
      ...
      if (targetProduct) {
        ...
        if (options.workType === 'rank') {
          ...
          try {
            const clickResult = await clickProduct(page, targetProduct, productCode, pageNum, products.length, options.keywordId, threadPrefix, products, options.workType);
            ...
            try {
              ...
              if (expandedTarget) {
                ...
                if (options.workType === 'rank') {
                  ...
                  // click 모드
                  try {
                    const clickResult = await clickProduct(page, expandedTarget, productCode, pageNum, products.length, options.keywordId, threadPrefix, products, options.workType);
