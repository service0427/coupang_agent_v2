...
class SharedCacheManager {
  constructor(config = {}) {
    // 상대 경로를 절대 경로로 변환 (심볼릭 링크 생성을 위해)
    this.basePath = path.resolve(config.basePath || './browser-data');
   ...
   * 캐시 크기 관리 (최대 2GB 제한)
   */
  async manageCacheSize(maxSizeGB = 2) {
    try {
      const maxSizeBytes = maxSizeGB * 1024 * 1024 * 1024;

      // 캐시 크기 확인
      const { execSync } = require('child_process');
      const sizeOutput = execSync(`du -sb "${this.sharedCachePath}" 2>/dev/null || echo "0"`, { encoding: 'utf8' });
      const currentSize = parseInt(sizeOutput.split('\t')[0]) || 0;

      if (currentSize > maxSizeBytes) {
        console.log(`🧹 캐시 크기 초과 (${(currentSize / 1024 / 1024 / 1024).toFixed(2)}GB > ${maxSizeGB}GB) - 정리 시작`);

        // 오래된 캐시 파일 정리
        const cacheTypes = ['Cache', 'Code Cache', 'GPUCache'];
        ...
        for (const cacheType of cacheTypes) {
          ...
          try {
            // 3일 이상 접근되지 않은 파일 삭제
            execSync(`find "${cachePath}" -type f -atime +3 -delete 2>/dev/null || true`);
          } catch (e) {
            // 무시
          }
        }
        ...
        // 정리 후 크기 확인
        const newSizeOutput = execSync(`du -sb "${this.sharedCachePath}" 2>/dev/null || echo "0"`, { encoding: 'utf8' });
        const newSize = parseInt(newSizeOutput.split('\t')[0]) || 0;
        const freedMB = (currentSize - newSize) / 1024 / 1024;

        console.log(`✅ 캐시 정리 완료: ${freedMB.toFixed(1)}MB 확보`);

        return { cleaned: true, freedBytes: currentSize - newSize };
      }

      return { cleaned: false, currentSizeGB: currentSize / 1024 / 1024 / 1024 };
    } catch (error) {
      console.error('캐시 크기 관리 실패:', error.message);
...
// =====================================================
// 프로세스 종료 핸들러 (api-mode.js에서 통합 관리)
// =====================================================
// 주의: SIGINT/SIGTERM 핸들러는 api-mode.js에서 통합 관리함
// 여기서 중복 등록하면 핸들러 충돌로 Ctrl+C가 안 먹힐 수 있음
