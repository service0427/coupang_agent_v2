...
async function extractProductList(page, productCode, keywordId = null, threadPrefix = '', pageNum = 1) {
  ...
  const products = await page.$$eval('#product-list > li[data-id], #product-list > li', (items, data) => {
    ...
    return items.map((item, index) => {
      ...
      // 배송 타입 분류
      // 배송 관련 이미지 찾기 (우선순위: 배지 클래스 > alt 속성)
      let deliveryImageUrl = null;
      
      // 1. 먼저 ImageBadge 클래스 내의 img 찾기
      const badgeImg = item.querySelector('[class*="ImageBadge"] img[src]');
      if (badgeImg) {
        const src = badgeImg.getAttribute('src');
        if (src && src.trim() !== '') {
          deliveryImageUrl = src;
        }
      }
      
      // 2. ImageBadge에서 못 찾으면 alt="로켓배송" 또는 alt="로켓직구" 찾기
      if (!deliveryImageUrl) {
        const altImg = item.querySelector('img[alt="로켓배송"], img[alt="로켓직구"]');
        if (altImg) {
          const src = altImg.getAttribute('src');
          if (src && src.trim() !== '') {
            deliveryImageUrl = src;
          }
        }
