      ...
      for (const selector of productSelectors) {
        try {
          ...
          if (element) {
            ...
            // 클릭 직전 최종 target 속성 확인 및 변경
            console.log(`${threadPrefix}    ${idPrefix}🔧 클릭 직전 target 속성 최종 확인...`);
            const finalTargetCheck = await element.evaluate(el => {
              if (el.tagName === 'A') {
                const originalTarget = el.getAttribute('target');
                console.log(`클릭 직전 target 속성:`, originalTarget);

                // 강력한 target 변경 (여러 방법 동시 적용)
                if (originalTarget === '_blank') {
                  ...
                  console.log(`변경 후 target 속성:`, finalTarget);
                ...
                return {
                  ...
                  success: originalTarget !== '_blank'
                };
              }
              return { changed: false, success: true };
            });

            if (finalTargetCheck.changed) {
              console.log(`${threadPrefix}    ${idPrefix}🔄 클릭 직전 target 변경: ${finalTargetCheck.original} → ${finalTargetCheck.final}`);
            }

            if (!finalTargetCheck.success) {
              console.log(`${threadPrefix}    ${idPrefix}⚠️ target 속성 변경 최종 실패! 강제 변경 시도...`);

              // 최후의 수단: 엘리먼트를 다시 찾아서 변경
              try {
                await page.evaluate(() => {
                  // 모든 링크의 target을 강제로 변경
                ...
                console.log(`${threadPrefix}    ${idPrefix}🔧 전체 페이지 target 속성 강제 변경 완료`);
              } catch (forceError) {
                console.log(`${threadPrefix}    ${idPrefix}❌ 강제 변경도 실패: ${forceError.message}`);
              }
            }
