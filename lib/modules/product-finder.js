...
async function extractProductList(page, productCode, keywordId = null, threadPrefix = '', pageNum = 1) {
  ...
  const extractedCodes = products
    .filter(p => p.productCode && !p.isAd) // 광고 제외, 코드가 있는 것만
    .map(p => p.productCode);
...
function findTargetProduct(products, productCode, keywordId = null, threadPrefix = '') {
  ...
  if (adProduct) {
    ...
    console.log(`${threadPrefix}    ${idPrefix}   📍 광고 상품 정보: ${adProduct.productName} (${adProduct.rank}순위)`);
    return null;
  } else {
    ...
    if (partialMatches.length > 0) {
      ...
      partialMatches.slice(0, 3).forEach(p => {
        console.log(`${threadPrefix}    ${idPrefix}   - ${p.productCode} (${p.isAd ? '광고' : '일반'}): ${p.productName.substring(0, 30)}...`);
      });
    }
