...
function findTargetProduct(products, productCode, keywordId = null, threadPrefix = '', itemId = null, vendorItemId = null) {
  ...
  // [DEBUG] 매칭 시도 로깅
  console.log(`${threadPrefix}    ${idPrefix}🔍 매칭 시도: product_id=${productCode}, item_id=${itemId}, vendor_item_id=${vendorItemId}`);
  ...
  if (found) {
    ...
    console.log(`${threadPrefix}    ${idPrefix}   매칭된 정보: product_id=${found.productCode}, item_id=${found.itemId}, vendor_item_id=${found.vendorItemId}`);
    return found;
  }
  ...
  if (productCodeMatches.length > 0) {
    ...
    productCodeMatches.forEach(p => {
      console.log(`${threadPrefix}    ${idPrefix}   후보: item_id=${p.itemId}, vendor_item_id=${p.vendorItemId}, href=${p.href.substring(0, 100)}...`);
    });
  }
