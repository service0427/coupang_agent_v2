...
function findTargetProduct(products, productCode, keywordId = null, threadPrefix = '', itemId = null, vendorItemId = null) {
  ...
  // 확장된 매칭 로직: product_id + item_id + vendor_item_id
  const found = products.find(p => {
    // 1. product_code는 필수 매칭
    if (p.productCode !== productCode) return false;
    
    // 2. item_id와 vendor_item_id 매칭 (null이 아닌 경우만 체크)
    if (itemId !== null && p.itemId !== null && p.itemId !== itemId) return false;
    if (vendorItemId !== null && p.vendorItemId !== null && p.vendorItemId !== vendorItemId) return false;
  ...
  if (found) {
    ...
    if (itemId !== null || found.itemId !== null) {
      const itemMatch = itemId === found.itemId ? '✓' : (itemId !== null && found.itemId !== null ? '✗' : '(비교 안함)');
      console.log(`${threadPrefix}    ${idPrefix}   ├─ item_id: ${found.itemId || 'null'} ${itemMatch}`);
    }
    if (vendorItemId !== null || found.vendorItemId !== null) {
      const vendorMatch = vendorItemId === found.vendorItemId ? '✓' : (vendorItemId !== null && found.vendorItemId !== null ? '✗' : '(비교 안함)');
      console.log(`${threadPrefix}    ${idPrefix}   └─ vendor_item_id: ${found.vendorItemId || 'null'} ${vendorMatch}`);
    }
    return found;
  }
  
  // [DEBUG] 매칭 실패 시 상세 분석
  const productCodeMatches = products.filter(p => p.productCode === productCode);
  if (productCodeMatches.length > 0) {
    console.log(`${threadPrefix}    ${idPrefix}⚠️ product_id는 일치하지만 item_id/vendor_item_id가 다름:`);
    productCodeMatches.forEach((p, idx) => {
      console.log(`${threadPrefix}    ${idPrefix}   후보 ${idx + 1}:`);
      console.log(`${threadPrefix}    ${idPrefix}     ├─ product_id: ${p.productCode} ✓`);
      console.log(`${threadPrefix}    ${idPrefix}     ├─ item_id: ${p.itemId || 'null'} ${itemId !== null && p.itemId !== itemId ? '✗ (불일치)' : ''}`);
      console.log(`${threadPrefix}    ${idPrefix}     ├─ vendor_item_id: ${p.vendorItemId || 'null'} ${vendorItemId !== null && p.vendorItemId !== vendorItemId ? '✗ (불일치)' : ''}`);
      console.log(`${threadPrefix}    ${idPrefix}     └─ 상품명: ${p.productName.substring(0, 50)}...`);
    });
  } else {
