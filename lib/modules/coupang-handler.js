...
async function searchAndClickProduct(page, options = {}) {
  ...
  try {
    ...
    // networkMonitor 제거됨 - 실제로 전달되지 않아 작동하지 않았음
    
    // monitor 옵션 제거됨
    
    // 쿠키 추적 제거됨
    
    // trafficManager 제거됨 - 실제로 전달되지 않아 작동하지 않았음
  }
...
// =====================================================
// search-mode-handler.js에서 통합
// =====================================================

/**
 * 메인 페이지에서 검색하여 검색 결과 페이지로 이동
 */
async function executeDirectMode(page, searchQuery, options = {}) {
  ...
  try {
    ...
    const searchInputSelector = 'input.headerSearchKeyword[name="q"]';
    ...
    // 3. 페이지 로드 후 3초 대기
    console.log(`${threadPrefix} ${idPrefix}⏳ 페이지 안정화 대기 (3초)...`);
    await page.waitForTimeout(3000);
    ...
    // 5. 타이핑 후 잠시 대기
    console.log(`${threadPrefix} ${idPrefix}⏳ 1초 대기 중...`);
    ...
    // 7. listSize=72 파라미터 추가하여 재이동
    const currentUrl = page.url();
    let modifiedUrl;

    if (currentUrl.includes('listSize=')) {
      // 이미 listSize가 있으면 72로 변경
      modifiedUrl = currentUrl.replace(/listSize=\d+/, 'listSize=72');
    } else {
      // listSize가 없으면 추가
      const separator = currentUrl.includes('?') ? '&' : '?';
      modifiedUrl = currentUrl + `${separator}listSize=72`;
    }

    console.log(`${threadPrefix} ${idPrefix}📦 노출 갯수 72개로 변경...`);
    console.log(`${threadPrefix} ${idPrefix}📍 변경된 URL: ${modifiedUrl}`);
    ...
    console.log(`${threadPrefix} ${idPrefix}✅ 72개 노출 페이지 로드 완료`);
