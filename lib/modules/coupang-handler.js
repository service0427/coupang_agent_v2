    ...
    for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
      ...
      try {
        ...
        if (error.errorType === 'blocked' || 
          ...
          if (options.workType === 'rank' && pageNum >= 5) {
            console.log(`${threadPrefix} ⚠️ HTTP2 차단 감지 - ${pageNum}페이지에서 차단되었지만 충분한 데이터 수집`);
            console.log(`${threadPrefix}    ✅ rank 모드 부분 성공으로 처리 (${pageNum}페이지까지 검색)`);
            ...
            result.errorMessage = `${pageNum}페이지 검색 후 차단 (부분 성공)`;
            result.referer = lastSearchPageUrl;
            ...
            if (options.workType === 'rank') {
              console.log(`${threadPrefix}    ⚠️ rank 모드 조기 차단 (${pageNum}페이지) - 실패 처리`);
            }
